...
class CategoryRepository(RepositoryProtocol):
    ...
    async def get(self, id: str) -> Optional[Category]:
        if (category := await self._collection.find_one({"_id": id})) is not None:
            return Category(**category)
        return None

    async def delete(self, id: str) -> bool:
        delete_result = await self._collection.delete_one({'_id': id})
    ...
    async def update(self, id: str, entity: Dict[str, Any]) -> bool:
        update_result = await self._collection.update_one({"_id": id}, {"$set": entity})
